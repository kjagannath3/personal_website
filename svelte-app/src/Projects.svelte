<script>
    import Navbar from "./Navbar.svelte";
</script>


<Navbar />
<p class="content-title">
    My projects
</p>
<div class="content">
    <div class ="text-picture-container">
        <div class="text-container">
        <h2>Check out some of my work below!</h2>
            <p>
                The projects you see below are either from my courses at Georgia Tech, or from my personal 
                time. Unfortunately, for the vast majority of my class-based projects, I am unable to show my 
                entire source code due to academic integrity policies. For the code I am allowed to share, I have 
                included the corresponding github repository next to the description. Have fun!
            </p>	
        </div>
    </div>

</div>



<style>
    .demos-title {
         text-align: center;
         font-size: 30px;
         font-weight: 700;
         margin-bottom: 20px;
 
    }
</style>